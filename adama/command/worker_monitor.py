...
def is_up(cid):
    ...
    """
    try:
        info = DOCKER_CLIENT.inspect_container(cid)
...
def is_ready(cid):
    ...
    if not is_up(cid):
        return False
    cat = DOCKER_CLIENT.exec_create(
        cid, ['cat', '/busy'], stdout=True, stderr=True)
    DOCKER_CLIENT.exec_start(cat['Id'])
    exit_code = DOCKER_CLIENT.exec_inspect(cat['Id'])['ExitCode']
